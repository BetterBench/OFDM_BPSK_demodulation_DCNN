...
if __name__ == '__main__':
    ...
    #训练集和训练标签
    train_featurestemp = matfile['rxTraining']
    train_labelstemp = matfile['rxTrainingLabel']
    #验证集和验证标签
    valid_featurestemp = matfile['rxValidation']
    valid_labelstemp = matfile['rxTrainingLabel']
    #测试集和测试标签
    test_featurestemp = matfile['rxTest']
    test_labelstemp = matfile['rxTestLabel']

    # 从float64转换类型为float32
    train_features = tf.convert_to_tensor(train_featurestemp,dtype=tf.float32)
    train_labels = tf.convert_to_tensor(train_labelstemp,dtype=tf.float32)

    valid_features = tf.convert_to_tensor(valid_featurestemp,dtype=tf.float32)
    valid_labels = tf.convert_to_tensor(valid_labelstemp,dtype=tf.float32)

    test_features = tf.convert_to_tensor(test_featurestemp,dtype=tf.float32)
    test_labels = tf.convert_to_tensor(test_labelstemp,dtype=tf.float32)
