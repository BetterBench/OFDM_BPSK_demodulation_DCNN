...
class DCNN:
    ...
    def fc_layers(conv):
        ...
        # fc4
        with tf.name_scope('fc3'):
            fc4w = tf.Variable(tf.truncated_normal([8, 225], dtype=tf.float32, stddev=1e-1), name='weights')
            fc4b = tf.Variable(tf.constant(1.0, shape=[225], dtype=tf.float32), trainable=True, name='biases')
...
#         with tf.name_scope('fc1'):
#             shape = int(np.prod(conv.get_shape()[1:]))
#             fc1w = tf.Variable(tf.truncated_normal([shape, 2], dtype=tf.float64, stddev=1e-1), name='weights')
#             fc1b = tf.Variable(tf.constant(0.0, shape=[2], dtype=tf.float64), trainable=True, name='biases')
